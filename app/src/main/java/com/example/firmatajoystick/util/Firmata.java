...
//	LinkedList<Byte> buffer = new LinkedList<>();
	...
	public Firmata(UsbSerialPort com) {
		this.com = com;
	}
	...
	public void sendAnalog(int pin, int value) {
		try {
			com.write(encode(ANALOG_MESSAGE, pin, value), 0);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void sendDigital(int pin, int value) {
		try {
			com.write(encode(DIGITAL_MESSAGE, pin, value), 0);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void sendRequestVersion() {
		try {
			com.write(new byte[] {(byte)REPORT_VERSION}, 0);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void sendSystemReset() {
		try {
			com.write(new byte[] {(byte)SYSTEM_RESET}, 0);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	/**
	 * public void sendStepMotor(int step)
	 */
	public void sendStepMotor(int step) {
		byte[] data = new byte[4];
		data[0] = (byte)(step >> 8*3);
		data[1] = (byte)(step >> 8*2);
		data[2] = (byte)(step >> 8*1);
		data[3] = (byte)(step >> 8*0);
		sendSysex((byte)STEP_MOTOR_DATA, (byte)4, data);
	}
	/*
